...
function back(){
    ...
    if(myTimer = -1){
        myTimer = setInterval(() => {
            ...
            window.scrollTo(0,scrollTo) //这是值  是指离网页顶部0px   
        }, 10);
    }
}
...
var aLink =document.getElementsByClassName("search_logo_lk")[0]
// var list =document.getElementById("search")
//当页面滚动时，触发这个函数
//追求两个目的 1.上滾到目标位置时，产生定位，脱离文档流就可以悬浮 。
//            2.下滾到目标距离时，定位取消，将间隔也取消
window.onscroll=function(){
    var panduan =document.documentElement.scrollTop ||document.body.scrollTop
    //这个是获取滚动窗口时的移动距离，将这个距离反馈到panduan里面去

    if(panduan>=300){
    //当滚动窗口达到300距离时，我们需要启动定位，来造成粘性效果
    //首先需要获得  需要定位的元素
    nav.style.position="fixed"
    nav.style.top="-14px"
    nav.style.borderBottom="solid 2px red"
    nav.style.backgroundColor="#fff"
    nav.style.paddingBottom="10px"
    
    
    aLink.style.opacity=1
    // $(".search_logo_lk")[0].css({
    //     "opacity":"1"
    // })
    console.log(aLink);
